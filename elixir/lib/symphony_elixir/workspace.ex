defmodule SymphonyElixir.Workspace do
  ...
  def create_for_issue(issue_or_identifier, worker_host \\ nil) do
    ...
    try do
      ...
      with {:ok, workspace} <- workspace_path_for_issue(safe_id, worker_host),
           ...
           :ok <- write_issue_manifest(workspace, issue_or_identifier, issue_context, worker_host) do
        {:ok, workspace}
      end
    rescue
  ...
  defp write_issue_manifest(workspace, %{} = issue_or_identifier, issue_context, nil) do
  ...
  defp write_issue_manifest(workspace, %{} = issue_or_identifier, issue_context, worker_host)
    ...
    with {:ok, encoded} <- Jason.encode(manifest, pretty: true) do
      script =
        [
          remote_shell_assign("workspace", workspace),
          "mkdir -p \"$workspace/.symphony\"",
          "cat > \"$workspace/.symphony/issue.json\" <<'__SYMPHONY_ISSUE_JSON__'",
          encoded,
          "__SYMPHONY_ISSUE_JSON__"
        ]
        |> Enum.join("\n")

      case run_remote_command(worker_host, script, Config.settings!().hooks.timeout_ms) do
        {:ok, {_output, 0}} -> :ok
        {:ok, {output, status}} -> {:error, {:issue_manifest_write_failed, worker_host, status, output}}
        {:error, reason} -> {:error, {:issue_manifest_write_failed, reason}}
      end
    else
      {:error, reason} -> {:error, {:issue_manifest_encode_failed, reason}}
    end
  end

  defp write_issue_manifest(_workspace, _issue_or_identifier, _issue_context, _worker_host), do: :ok
